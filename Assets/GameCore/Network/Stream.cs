...
namespace GameCore.Network.Stream {
	...
	public static class OutputStreamHelper {
		...
		public static void WriteStorySceneCameraCommand(IDataOutputStream stream, CameraAnimCommand command) {
			stream.WriteBoolean(command.isEased);
			stream.WriteSingle(command.timeOffset);
			if (command.isEased) {
		...
		public static void WriteStorySceneObjectCommand(IDataOutputStream stream, ObjectAnimCommand command) {
			stream.WriteBoolean(command.isEased);
			stream.WriteSingle(command.timeOffset);
			if (command.isEased) {
	...
	public static class InputStreamHelper {
		...
		public static CameraAnimCommand ReadStorySceneCameraCommand(IDataInputStream stream) {
			CameraAnimCommand ret;
			bool isEasedCmd = stream.ReadBoolean();
			float timeOffset = stream.ReadSingle();
			if (isEasedCmd) {
		...
		public static ObjectAnimCommand ReadStorySceneObjectCommand(IDataInputStream stream) {
			ObjectAnimCommand ret;
			bool isEasedCmd = stream.ReadBoolean();
			float timeOffset = stream.ReadSingle();
			if (isEasedCmd) {
